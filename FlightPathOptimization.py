...
class FlightPathOptimization(object):
    ...
    def schedule_cost(self, sol):
        ...
        for d in range(len(sol) / 2):
            ...
            if latestarrival < self.get_minutes(outbound[1]): latestarrival = self.get_minutes(outbound[1])
            if earliestdep > self.get_minutes(returnf[0]): earliestdep = self.get_minutes(returnf[0])
        ...
        for d in range(len(sol) / 2):
            ...
            outbound = self.flights[(origin, self.destination)][int(sol[d * 2])]
            returnf = self.flights[(self.destination, origin)][int(sol[d * 2 + 1])]
            totalwait += latestarrival - self.get_minutes(outbound[1])
            totalwait += self.get_minutes(returnf[0]) - earliestdep
